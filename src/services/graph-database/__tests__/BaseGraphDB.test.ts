    ...
    return {
      query: vi.fn().mockImplementation(function (sql: string, params?: any[]) {
        if (sql.includes("SELECT * FROM nodes")) {
          ...
          sql.includes("SELECT key, value FROM relationship_properties")
        ) {
          return {
            values: [{ key: "since", value: '"2020-01-01"' }],
          };
...
describe("BaseGraphDB", () => {
  ...
  describe("事务操作", () => {
    ...
    it("应该处理事务中的错误", async () => {
      ...
      // 操作将抛出错误
      await expect(
        (testDb as any).withTransaction(async () => {
          await mockDb.run("INSERT INTO test_table VALUES (1)");
          return true;
        })
      ).rejects.toThrow();
  ...
  describe("节点操作", () => {
    ...
    it("删除不存在的节点应该抛出错误", async () => {
      ...
      await expect(db2.deleteNode("not-exists")).rejects.toThrow(
        NodeNotFoundError
      );
  ...
  describe("边操作", () => {
    ...
    it("添加边时源节点不存在应该抛出错误", async () => {
      ...
      const queryMock = vi
        .fn()
        .mockImplementation(function (sql: string, params?: any[]) {
          if (
            sql.includes("SELECT 1 FROM nodes") &&
            params?.[0] === "not-exists"
          ) {
            return { values: [] };
          }
    ...
    it("删除不存在的边应该抛出错误", async () => {
      ...
      await expect(db2.deleteEdge("not-exists")).rejects.toThrow(
        EdgeNotFoundError
      );
      expect(queryMock).toHaveBeenCalledWith(
  ...
  describe("查询操作", () => {
    ...
    it("源节点不存在时查找路径应该抛出错误", async () => {
      ...
      await expect(db2.findPath("not-exists", "node2")).rejects.toThrow(
        NodeNotFoundError
      );
      expect(queryMock).toHaveBeenCalledWith(
        expect.stringContaining("SELECT 1 FROM nodes"),
    ...
    it("目标节点不存在时查找路径应该抛出错误", async () => {
      ...
      await expect(db2.findPath("node1", "not-exists")).rejects.toThrow(
        NodeNotFoundError
      );
      expect(queryMock).toHaveBeenCalledWith(
        expect.stringContaining("SELECT 1 FROM nodes"),
  ...
  describe("抽象方法", () => {
    it("importData应该抛出错误", async () => {
      await expect(db.importData(new Uint8Array(0))).rejects.toThrow(
        DatabaseError
      );
    });
    ...
    it("restoreFromBackup应该抛出错误", async () => {
      await expect(db.restoreFromBackup("backup-id")).rejects.toThrow(
        DatabaseError
      );
    });
  ...
  describe("Structured Relationship Validation", () => {
    it("validateStructuredRelationships应该返回验证结果", async () => {
    ...
    it("validateStructuredRelationships在数据库未初始化时应该抛出错误", async () => {
      const uninitializedDb = new TestGraphDB();
      await expect(
        uninitializedDb.validateStructuredRelationships()
